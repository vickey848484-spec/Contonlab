...
(function (global) {
    ...
    el.innerHTML = `
      <div class="share-card" id="share-card-inner">
        <div class="share-card-inner">
          ...
          <div class="share-tagline">${global.Cantonese.pick(mbtiType.tagline) || ''}</div>
          <div class="share-desc">${global.Cantonese.pick(mbtiType.desc) || ''}</div>
        </div>
  ...
  function composeText(level, mbtiType, mbtiCode) {
    ...
    return [
      `我的粵語人格：${mbtiCode} · ${pick(mbtiType.title) || ''}`,
      `${pick(mbtiType.tagline) || ''}`, '',
      `${pick(mbtiType.desc) || ''}`, '',
      `測一測你的 → 識講粵語`,
    ].join('\n');
  }
  ...
  function downloadPoster({ mbtiCode, mbtiType, level, ref }) {
    ...
    ctx.fillText(global.Cantonese.pick(mbtiType.title), W / 2, 540);

    // tagline
    ctx.fillStyle = '#0066ff';
    ctx.font = 'bold 26px "PingFang SC", sans-serif';
    ctx.fillText(global.Cantonese.pick(mbtiType.tagline) || '', W / 2, 590);

    // 描述（換行）
    ctx.fillStyle = 'rgba(255,255,255,0.9)';
    ctx.font = '22px "PingFang SC", sans-serif';
    ctx.textAlign = 'left';
    wrapText(ctx, global.Cantonese.pick(mbtiType.desc), 60, 660, W - 120, 36);
    ctx.textAlign = 'center';
